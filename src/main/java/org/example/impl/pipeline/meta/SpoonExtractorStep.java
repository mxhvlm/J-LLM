...
import org.example.impl.JLLMConfig;
...
import org.example.impl.pipeline.code._package.ExtractorStep;
import org.example.impl.pipeline.code._package.LoaderStep;
import org.example.impl.pipeline.code._package.TransformerStep;
...
public class SpoonExtractorStep extends AbstractNeo4jMetaStep {
    ...
    public INeo4jProvider process(INeo4jProvider neo4JProvider) {
        ...
        Pipeline<CodeModel, TransformResult> typePipeline = Pipeline
                .start(new org.example.impl.pipeline.code.type.ExtractorStep())
                .then(new org.example.impl.pipeline.code.type.TransformerStep())
                .then(new org.example.impl.pipeline.code.type.LoaderStep(neo4JProvider))
                .build();


        Pipeline<CodeModel, TransformResult> fieldPipeline = Pipeline
                .start(new org.example.impl.pipeline.code.type.ExtractorStep())
                .then(new org.example.impl.pipeline.code.field.ExtractorStep())
                .then(new org.example.impl.pipeline.code.field.TransformerStep())
                .then(new org.example.impl.pipeline.code.field.LoaderStep(neo4JProvider))
                .build();

        Pipeline<CodeModel, TransformResult> methodPipeline = Pipeline
                .start(new org.example.impl.pipeline.code.type.ExtractorStep())
                .then(new org.example.impl.pipeline.code.method.ExtractorStep())
                .then(new org.example.impl.pipeline.code.method.TransformerStep())
                .then(new org.example.impl.pipeline.code.method.LoadStep(neo4JProvider))
                .build();
//
//        Pipeline<CodeModel, TransformResult> methodPipeline = Pipeline
